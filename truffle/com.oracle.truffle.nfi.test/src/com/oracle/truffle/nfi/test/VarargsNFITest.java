...
public class VarargsNFITest extends NFITest {
    ...
    private static class SimplePrintfRoot extends RootNode {
    ...
    private static class MultiPrintfRoot extends RootNode {
        ...
        public Object execute(VirtualFrame frame) {
            for (int i = 0; i < specs.length; i++) {
                ...
                checkRet(specs[i].expectedRet, ret);
            }
            return null;
        }

        @TruffleBoundary
        private static void checkRet(String expected, String actual) {
    ...
    public void testMultiTypesPrintf() {
        ...
        MultiPrintfRoot root = new MultiPrintfRoot(
                        ...
                        new PrintfSpec("(nil) 8472", "([sint8], uint64, string, ...pointer, sint64) : sint32", "%p %i", nil, 8472),
                        ...
                        new PrintfSpec("(nil) world", "([sint8], uint64, string, ...pointer, string) : sint32", "%p %s", nil, "world"),
    ...
    public void testVariableArgCountPrintf() {
        ...
        MultiPrintfRoot root = new MultiPrintfRoot(
                        ...
                        new PrintfSpec("42 x (nil)", "([sint8], uint64, string, ...sint64, string, pointer) : sint32", "%i %s %p", 42, "x", nil),
                        new PrintfSpec("42 x (nil) 42.00", "([sint8], uint64, string, ...sint64, string, pointer, double) : sint32", "%i %s %p %2.2f", 42, "x", nil, 42),
                        new PrintfSpec("x", "([sint8], uint64, string, ...string) : sint32", "%s", boxedX),
                        new PrintfSpec("x (nil)", "([sint8], uint64, string, ...string, pointer) : sint32", "%s %p", boxedX, nil),
                        new PrintfSpec("x (nil) 42.00", "([sint8], uint64, string, ...string, pointer, double) : sint32", "%s %p %2.2f", boxedX, nil, 42),
                        new PrintfSpec("x (nil) 42.00 42", "([sint8], uint64, string, ...string, pointer, double, sint64) : sint32", "%s %p %2.2f %i", boxedX, nil, 42, 42));
        run(root);
    }
}
